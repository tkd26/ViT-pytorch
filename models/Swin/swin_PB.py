...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., config=None):
        ...
        self.fc1 = ElementWiseLinear(in_features, hidden_features, config)
        self.act = act_layer()

        # self.fc2 = nn.Linear(hidden_features, out_features)
        self.fc2 = ElementWiseLinear(hidden_features, out_features, config)
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., config=None):
        ...
        self.qkv = ElementWiseLinear(dim, dim * 3, config)
        self.attn_drop = nn.Dropout(attn_drop)
        # self.proj = nn.Linear(dim, dim)
        self.proj = ElementWiseLinear(dim, dim, config)
