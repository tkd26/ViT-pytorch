...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., config=None, task=None):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features

        # self.fc1 = nn.Linear(in_features, hidden_features)
        self.fc1 = PiggybackFC(in_features, hidden_features, config, task=task)
        self.act = act_layer()

        # self.fc2 = nn.Linear(hidden_features, out_features)
        self.fc2 = PiggybackFC(hidden_features, out_features, config, task=task)
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., config=None, task=None):
        ...
        self.qkv = PiggybackFC(dim, dim * 3, config, task=task)
        self.attn_drop = nn.Dropout(attn_drop)
        # self.proj = nn.Linear(dim, dim)
        self.proj = PiggybackFC(dim, dim, config, task=task)
    ...
    def forward(self, x, task, mask=None):
        ...
        """
        B_, N, C = x.shape
        qkv = self.qkv(x, task).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        # qkv = self.qkv(x, task).reshape(B_, N, 3, self.num_heads, C // self.num_heads)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, config=None, task=None):
        super().__init__()
        self.dim = dim
        ...
        self.attn = WindowAttention(
            ...
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, config=config, task=task)

        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, config=config, task=task)
    ...
    def forward(self, x, task):
        ...
        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, task, mask=self.attn_mask)  # nW*B, window_size*window_size, C
        ...
        x = x + self.drop_path(self.mlp(self.norm2(x), task))
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False, config=None, task=None):
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, config, img_size, num_classes, task, **kwargs):
        super().__init__()
        ...
        if self.multi_head:
            self.head = nn.ModuleList([nn.Linear(self.num_features, num_class) for num_class in num_classes])
        else:
